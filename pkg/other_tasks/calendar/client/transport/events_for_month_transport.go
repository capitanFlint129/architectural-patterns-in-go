...
type eventsForMonthClientTransport struct {
	url            *url.URL
	path           string
	httpMethod     string
...
func (c *eventsForMonthClientTransport) EncodeRequest(data types.DateHandlerData) (*http.Request, error) {
	params := url.Values{}
...
func (c *eventsForMonthClientTransport) DecodeResponse(r *http.Response) ([]types.Event, error) {
...
func NewEventsForMonthClientTransport(
	url *url.URL, path string, httpMethod string, errorTransport ErrorClientTransport, dateFormat string,
) EventsForMonthClientTransport {
	return &eventsForMonthClientTransport{
		url:            url,
		path:           path,
